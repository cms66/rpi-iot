...
install_local()
{
	...
	mpirun --version
	read -p "OpenMPI 4.0.3 - Local install finished, press enter to return to menu" input
}

# 2- Build/install on server
install_server()
{
	read -p "OpenMPI 4.0.3 - Server install TODO, press enter to return to menu" input
}

# 3- Install to run from server
install_client()
{
	read -p "OpenMPI 4.0.3 - Client install TODO, press enter to return to menu" input
}
